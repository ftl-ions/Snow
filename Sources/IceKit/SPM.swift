...
public enum SwiftExecutable {
    ...
    public static var toolchainPath: Path? = {
        ...
        let task = Task(
            executable: "swift", arguments: ["-v", tmpFile.string], stdout: WriteStream.null,
            stderr: capture)
        task.runSync()
        ...
        guard
            let compilerPath = capture.readAll().components(separatedBy: "\n").first(where: {
                $0.hasPrefix("/")
            }),
            let range = compilerPath.range(of: ".xctoolchain/")
        else {
            return nil
        }
    ...
    public static var version: SwiftToolsVersion? = {
        if let content = try? Task.capture("swift", "--version").stdout,
            let match = Regex("Swift version ([0-9]\\.[0-9](\\.[0-9])?)(-dev)? ").firstMatch(
                in: content),
...
public class SPM {
    ...
    public func build(
        release: Bool = false, buildOption: BuildOption? = nil, forwardArguments: [String] = []
    ) throws {
    ...
    public func generateTests(for packageTargets: [Package.Target]) throws {
        #if os(macOS)
            guard let version = SwiftExecutable.version,
                version >= SwiftToolsVersion(major: 4, minor: 1, patch: 0)
            else {
                throw IceError(
                    message: "test list generation only supported for Swift 4.1 and above")
            }
    ...
    public func dumpPackage() throws -> Data {
        ...
        guard let jsonStart = content.firstIndex(of: "{"),
            let data = String(content[jsonStart...]).data(using: .utf8)
        else {
    ...
    private func runSwift(args: [String], transformer: TransformerPair? = nil) throws {
        ...
        let stderr: WritableStream = transformer?.stderr ?? WriteStream.stderr
        let task = Task(
            executable: "swift", arguments: args, directory: directory.string, stdout: stdout,
            stderr: stderr)
        let result = task.runSync()
        transformer?.wait()
    ...
    private func captureSwift(args: [String]) throws -> CaptureResult {
        ...
        do {
            ...
            if error.captured.stderr.isEmpty {
                ...
                if let match = Regex("[eE]rror: (.*)$").firstMatch(in: error.captured.stderr),
                    let rest = match.captures[0]
                {
                    message = rest
                } else {
